    ...
    public ResponseEntity<String> addAntrag(@RequestParam long user_id, @RequestParam double wert) {
        ...
        if (user_id != 0 && wert > 0) {
            try {
                ...
                tmp.setBenutzerAuftrag(benutzertmp.get());
    ...
    public ResponseEntity<String> acceptAntrag(@RequestParam(value = "AuftragsID", required = true) long id) {
        ...
        if (id != 0) {
            try {
                ...
                var tmp_benutzer = benutzerrepository.findById(auftrag.get().getBenutzerAuftrag().getId());
                ...
                addRevision.setBenutzerRevision(tmp_benutzer.get());
